...
import { getNewMembersData } from "../services/api";
...
export const NewMembersPage = () => {
    ...
    useEffect(() => {
        const fetchNewMembersData = async () => {
            try {
                ...
                setNewMembersData(data.response);
    ...
    const membershipCharts = useMemo(() => {
        if (!newMembersData) return null;
        

        const tierCounts = newMembersData.reduce((acc, member) => {
            acc[member.tier] = (acc[member.tier] || 0) + 1;
            return acc;
        }, {});
        

        const statusByTier = newMembersData.reduce((acc, member) => {
            if (!acc[member.tier]) {
        ...
        const activeMembers = newMembersData.filter(m => m.status === 'Active' && m.expiration_date);
        ...
        const tierDurations = newMembersData.reduce((acc, member) => {
                    ...
                    {error ? (
                        ...
                        <>
                            <div className="total-clients-grid">
                                ...
                                <div className="total-chart">
                                    <h3 className="total-chart-title">New Membership Expiration Analysis</h3>
                                    <div style={{ position: 'relative', width: '500px', height: '200px' }}>
                                        <Doughnut 
                            ...
                            <div className="total-clients-grid">
                                <div className="total-chart">
                                    <h3 className="total-chart-title">Active vs Inactive New Members</h3>
                                    <div style={{ position: 'relative', width: '500px', height: '200px' }}>
                                        <Bar 
